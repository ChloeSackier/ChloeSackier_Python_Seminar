...
#calculating flux for uniform B-field
def Flux_u(phi):                             # Flux through loop at angle phi (angle between B and vector area)
    ctr = np.array((0.0, 0.0, 0.05))
    S = 0.5 * a                                      # Side of the loop
    Flux = 0
    for i in xrange(int(S / dx)):
        for j in xrange(int(S / dy)):
            ...
            B_u = mu_0 * I * (N / L)            # uniform B approximation from Ampere's Law
            dF = np.dot(B, dA)
            Flux_u += dF
    return Flux_u
